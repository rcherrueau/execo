...
class Virsh_Deployment(object):
    ...
    def enable_taktuk(self, ssh_key = None):
        ...
        EX.TaktukRemote('cat '+ssh_key+'.pub >> .ssh/authorized_keys; echo "Host * \n StrictHostKeyChecking no" >> .ssh/config; ',
    ...
    def install_packages(self, packages_list = None):
        ...
        cmd = ''
        base_cmd = 'export DEBIAN_MASTER=noninteractive ; apt-get update && apt-get install -y --force-yes '
        ...
        for p in check_version.processes():
            if p.stdout().strip() == '6.0':
                ...
                cmd += base_cmd+' -t squeeze-backports '+packages_list
            else:
                cmd += base_cmd+packages_list
        logger.debug(cmd)
    ...
    def create_bridge(self, bridge_name = 'br0'):
        ...
        cmd = 'br_if=`ip route |grep default |cut -f 5 -d \' \'`; echo "auto '+bridge_name+'" >> /etc/network/interfaces ; '+\
    ...
    def create_disk_image(self, disk_image = None, clean = False):
        ...
        EX.TaktukPut( self.hosts, disk_image, 
                         self.hosts, connexion_params = {'user': 'root',
                'host_rewrite_func': lambda host: re.sub("\.g5k$", ".grid5000.fr", host),        
                'taktuk_gateway': g5k_configuration['default_frontend'],}).run()
        if clean:
