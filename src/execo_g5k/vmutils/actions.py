...
def install_vms(vms):
    ...
    cmds = []
    for vm in vms:
        cmds.append('virt-install -d --import --connect qemu:///system --nographics --noautoconsole --noreboot'+ \
    ...
    return TaktukRemote('virsh --connect qemu:///system start {{[ vm["vm_id"] for vm in vms]}}', 
                 [ vm['host'] for vm in vms ])
...
def create_disks(vms, backing_file = '/tmp/vm-base.img', backing_file_fmt = 'raw'):
    ...
    return TaktukRemote('{{cmds}}', hosts)
...
def install_vms(vms, gateway = None):
    """ """    
    ...
    for host, actions in hosts_actions.iteritems():
        ...
        #install_actions.append(ParallelActions(actions))
...
def wait_vms_have_started(vms, gateway = None):
    
    vms = [vm['ip'] for vm in vms ] 
    ls_tries = 0
    ssh_open = False
    
    test_vm = TaktukRemote('ls', vms, connexion_params = {'taktuk_gateway': gateway})
