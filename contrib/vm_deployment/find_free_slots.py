...
class g5k_find_free_slots(object):
	...
	def __init__(self,walltime,resources,job_type='deploy',lan=None,
		...
		self.create_params(resources,job_type,lan)
		
		self.define_dates(walltime, weeks)
		self.generate_gantt()
		
		self.compute_hosts_freeslots()
		freeslots={}
...
#			print 'oargrid'
	...
	def compute_hosts_freeslots(self):
		self.logger.info('** Finding free slots for each hosts ...')
		self.hosts_freeslots={}	
		for site,clusters in self.params['sites'].iteritems():
			self.hosts_freeslots[site]={}						
			for cluster in clusters['clusters'].iterkeys():
				self.hosts_freeslots[site][cluster]={}	
				...
				for host in sorted(hosts_jobs.iterkeys()):
					...
					if len(jobs)>0:
						if jobs[0][0]>self.startdate_stamp:
							hosts_freeslots[host].append((self.startdate_stamp,jobs[0][0]))	
						for i in range(0,len(jobs)-1):
							hosts_freeslots[host].append((jobs[i][1],jobs[i+1][0]))
						if jobs[len(jobs)-1][1]<self.enddate_stamp:
							hosts_freeslots[host].append((jobs[len(jobs)-1][1],self.enddate_stamp))
					else:
						hosts_freeslots[host].append((self.startdate_stamp,self.enddate_stamp))
				self.hosts_freeslots[site][cluster]=hosts_freeslots
	...
	def generate_gantt(self):
		...
		frontends=[]
		for site in self.params['sites'].iterkeys():
			frontends.append(EX.Host(site+'.grid5000.fr'))
		self.logger.info('** Getting resources via oarstat')
		getgantt=EX.Remote(cmd_gantt, frontends,connexion_params=default_frontend_connexion_params).run()
	...
	def create_params(self,resources,job_type,lan):
		...
		for element, n_nodes in resources.iteritems():
			if element in g5k_clusters:
				self.params['sub_type']='clusters'
				if 'sites' not in self.params.keys():  
					self.params={'sites':{}}				
				site=API.get_cluster_site(element)
				if site not in self.params['sites'].keys():
					self.params['sites'][site]={'clusters': {}}
				cl_nodes=len(API.get_cluster_hosts(element))
				self.params['sites'][site]['clusters'][element]={'n_nodes': n_nodes, 'max_nodes': cl_nodes}
				if cl_nodes < n_nodes:
					self.logger.error('Not enough nodes on cluster \033[1m'+element+'\033[0m for your request')
					exit()
				
			elif element in g5k_sites:
				self.params['sub_type']='sites'
				site_nodes=0
				if 'sites' not in self.params.keys():
					self.params={'sites':{}}
				self.params['sites'][element]={'n_nodes':n_nodes,'clusters':{}}
				for cluster in API.get_site_clusters(element):
					max_nodes=len(API.get_cluster_hosts(cluster))					
					self.params['sites'][element]['clusters'][cluster]={'max_nodes': max_nodes}
					site_nodes+=n_nodes
				if site_nodes < n_nodes:
					self.logger.error('Not enough nodes on site \033[1m'+element+'\033[0m for your request')
					exit()
					
			else:	
				total_nodes=0
				self.params={'n_nodes':n_nodes,'sites':{}, 'sub_type': 'grid5000'}
				for site in g5k_sites:
					self.params['sites'][site]={'clusters':{}}
					for cluster in API.get_site_clusters(site):
						cl_nodes=len(API.get_cluster_hosts(cluster))
						self.params['sites'][site]['clusters'][cluster]={'max_nodes':cl_nodes}
						total_nodes+=n_nodes
				if total_nodes < n_nodes:
					self.logger.error('Not enough nodes on G5K platform for your request')
					exit()
			log+='\n* '+str(n_nodes)+' nodes on \033[1m'+element+'\033[0m'
		self.logger.info('** Ressources %s', log)
...
if __name__ == '__main__':
	if len(sys.argv)==1:
		...
		demo1=g5k_find_free_slots('6:00:00',{'stremi':20, 'chimint': 5, 'chicon': 5},
